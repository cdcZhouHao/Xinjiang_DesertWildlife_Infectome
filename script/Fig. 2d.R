...
# Please replace the path with your actual file location
file_path <- "C:\\Users\\zhouhao\\OneDrive\\Desktop\\Xinjiang_Altun_Project\\09_Virus_Diversity\\merged_Count.tsv"
df <- read.table(file_path, header = TRUE, sep = "\t", check.names = FALSE)

# Set species names as row names and transpose matrix (vegan requirement: rows are samples, columns are species)
...
################################################################################
#All pairwise comparisons (Dunn Test)
#install.packages("FSA")
library(FSA)

# Filter out groups with n=1 (optional; keeping them might cause errors or skip)
test_data <- plot_data %>% filter(Group != "GN")

# Perform Dunn Test (with p-value correction)
dunnTest(Shannon ~ Group, data = test_data, method = "bh")


# 1. Run test and assign results to an object
dunn_result <- dunnTest(Shannon ~ Group, data = test_data, method = "bh")

# 2. Extract the data frame (res) from results
dunn_table <- dunn_result$res


# Write to a Tab-delimited text file
write.table(dunn_table, "C:\\Users\\zhouhao\\OneDrive\\Desktop\\Xinjiang_Altun_Project\\09_Virus_Diversity\\Dunn_Test_Results.txt", sep = "\t", quote = FALSE, row.names = FALSE)
