...
# ------------------------------------------------------------------
input_file <- "C:\\Users\\zhouhao\\OneDrive\\Desktop\\Xinjiang_Altun_Project\\26_Virulence_Factors_v3\\VF_abundance_annotation.xlsx" 

# Please modify the sheet index according to your actual situation (e.g., write 3 for the third sheet)
df <- read_excel(input_file, sheet = 3) 

# Check if column names match, ensuring type, subtype, sum_tpm are included
# If column names differ, uncomment the line below and modify to your actual column names:
...
p <- ggraph(d, layout = "circlepack", weight = size) +
  ...
  # Add subcategory labels (abundance >= 50000)
  geom_node_text(
